...
def generate_response(
    model: str, conversation: List[Dict[str, str]], max_tokens: int = 1024
) -> str:
    ...
    try:
        if model == HIGH_TIER_MODEL:
            ...
            response = client.messages.create(
                model=model, max_tokens=max_tokens, messages=conversation
            )
...
def chat_with_99(args: argparse.Namespace) -> None:
    ...
    console.print(
        Panel(
            "Welcome to Chat99 with Multi-Model Routing!",
    ...
    if args.use_dynamic_routing:
        console.print(
            f"[bold yellow]Dynamic routing enabled using {args.router} router "
            f"with threshold {args.threshold}[/bold yellow]"
        )
    else:
        console.print(
            "[bold yellow]Using tiered model selection based on input complexity.[/bold yellow]"
        )
    ...
    while True:
        ...
        try:
            if args.use_dynamic_routing:
                ...
                routing_explanation = (
                    f"Routed using {args.router} router with threshold {args.threshold}"
                )
            else:
                complexity = (
                    "high"
                    if len(user_input.split()) > 50
                    else "mid" if len(user_input.split()) > 20 else "low"
                )
                model = (
                    HIGH_TIER_MODEL
                    if complexity == "high"
                    else MID_TIER_MODEL if complexity == "mid" else LOW_TIER_MODEL
                )
                content = generate_response(model, conversation)
                model_used = model
                routing_explanation = (
                    f"Selected {model} based on input complexity: {complexity}"
                )

            if content:
                console.print(
                    f"[bold green]Chat99 ([italic]{model_used}[/italic]):[/bold green] ",
                    end="",
                )
                console.print(content)
                console.print(
                    f"[bold yellow]Model Selection: {routing_explanation}[/bold yellow]"
                )
