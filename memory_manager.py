    ...
    def get_relevant_context(self, user_input: str) -> str:
        ...
        try:
            ...
            for memory in long_term_memories:
                relevant_info.append(f"Topic: {memory.get('topic', 'Unknown')}, Content: {memory.get('summary', 'No summary available')}")
        except Exception as e:
            print(f"Error retrieving long-term memories: {str(e)}")
        
        # If asked about previous discussions, provide a recap
        if any(phrase in user_input.lower() for phrase in ["what else", "what have we discussed", "what did we talk about"]):
